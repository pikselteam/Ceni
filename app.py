...
def save_data(dataframe):
    """Ги подредува податоците по азбука, ги израмнува колоните и ги зачувува назад во CSV фајлот."""
    try:
        ...
        # Проверка дали постои колоната 'Забелешка', ако не постои ја креираме празна
        if 'Забелешка' not in dataframe.columns:
            dataframe['Забелешка'] = ""
        else:
            dataframe['Забелешка'] = dataframe['Забелешка'].fillna("")
            
        # Зачувување во CSV со точно 6 колони за секој ред и чисти нови редови
        dataframe.to_csv(CSV_FILE, index=False, lineterminator='\n')
        st.success("Податоците се успешно зачувани, подредени и израмнети!")
    except Exception as e:
